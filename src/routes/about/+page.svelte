
<script>
   


    // Define a long text content to be displayed on the About page
    let longText = `
        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus vel odio ac eros rhoncus egestas. Nullam nec fermentum libero. Duis lacinia, tortor at pellentesque efficitur, lorem lorem convallis velit, in consectetur nunc odio nec libero. Vestibulum in tincidunt urna. Nullam tincidunt, orci a vulputate iaculis, nulla metus tincidunt nisi, non luctus sapien leo a elit. Fusce vestibulum euismod est, ut posuere elit posuere et. Nunc condimentum quam in tellus scelerisque, in vehicula libero vehicula. Integer vitae gravida odio. Quisque sit amet lacinia lectus. Sed tincidunt feugiat metus, sit amet sodales sapien varius eu. Sed eu libero nec arcu convallis blandit vel id ex. Donec sit amet convallis arcu. Quisque id vehicula neque. Donec tincidunt quam nec turpis blandit varius. Fusce non efficitur massa, id semper odio. Integer ut bibendum turpis. Sed vel nunc at libero interdum pharetra.

        Nam at malesuada est, nec finibus justo. Nulla a interdum libero. Fusce varius justo vel mauris varius, non bibendum eros venenatis. Sed ut feugiat eros. Cras interdum, sapien eu consequat facilisis, lectus metus tincidunt dolor, eu vulputate tellus purus sit amet nunc. Integer eu lorem quis ipsum mattis bibendum eget ac risus. Nullam rhoncus magna quis urna venenatis cursus. Maecenas ac gravida nulla. Sed quis tortor odio. Sed eleifend nisi id erat fermentum, a posuere quam convallis. Nunc eget felis nec orci venenatis semper. Nullam efficitur nec purus a suscipit. Sed auctor efficitur lacinia. Curabitur vitae posuere ex. Nullam dapibus, nunc ut tincidunt condimentum, odio urna viverra arcu, ac varius mi ex vel nulla. Maecenas nec posuere lorem. Etiam a orci felis.
        ...
    `;
</script>

<div class="container mx-auto p-4">
    <h1 class="text-3xl font-bold mb-4">About this website:</h1>
    <div class="bg-black p-4 rounded shadow-md">
        <!-- Display the long text content within a white container -->
        <p>{longText}</p>
    </div>
</div>
